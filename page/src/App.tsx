...
function App() {
	...
	useMemo(() => {
		if (data) {
			setRndObject(JSON.parse(JSONCrush.uncrush(data)) as RnDObject)
		}
	}, [data])
	...
	useEffect(() => {
		const intervafl = setInterval(() => {
			for (const connection of connectionList) {
				socket?.emit('variables:connection-values', [connection], (err: Error, res: unknown) => {
					if (err) {
						console.error(err)
					} else {
						setVariables((v) => {
	...
	const setRndSize = useCallback(
		(id: keyof RnDObject, width: string, height: string, position: { x: number; y: number }) => {
			const newRndData = {
				...rndData,
	...
	return (
		...
		>
			{Object.values(rndData).map((elm) => (
				...
				>
					{!edit &&
						TransformVariable(elm.data, variables, connectionList, addConnectionsList, elm.size?.height ?? '70px')}
					{edit && (
						<div>
							<button style={{ backgroundColor: 'red' }} onClick={() => removeElement(elm.id)}>
								X
							</button>
							<input
			...
			<div className={style.editRow}>
				...
				<button className={edit ? style.editButtonActive : style.editButton} onClick={() => setEdit(!edit)}>
					✎
				</button>
...
function TransformVariable(
	...
	if (!connectionsList.some((val) => connectionLabel == val)) {
		...
		return NewLine(key.replace(fullId, 'N/A'), height)
	}
	...
	const connectionVariable = variables[connectionLabel as keyof typeof variables]
...
function NewLine(data: string, height: string) {
	...
	const fontSize = isNaN(fullH) ? `${70 / lines.length}px` : `${fullH / lines.length}px`
